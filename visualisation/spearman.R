...
cor_method = "spearman"
correlation_matrix <- cor(eqtls$beta, method = cor_method)
correlation_matrix

correlation_matrix = as_tibble(correlation_matrix) %>% 
  mutate(dataset = rownames(correlation_matrix)) %>% 
  select(dataset, everything())
write_tsv(correlation_matrix, "spearman_corr.tsv")
...
pheatmap::pheatmap(correlation_matrix, fontsize=8, cluster_rows = T, cluster_cols = T, cellwidth = 10, cellheight = 8,
                   ...
                   filename = "figures/spearman_heatmap.pdf", width = 20, height = 14)
