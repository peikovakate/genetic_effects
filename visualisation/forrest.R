...
# extract the gene from effects file
...
metadata = distinct(metadata, gene_name, gene_id)
effects = left_join(effects, metadata, by=c("molecular_trait_id"="gene_id"))
rbms1_effects = effects %>% filter(gene_name == "RBMS1")
rbms1_effects %>% write_tsv("rbms1_effects.tsv")
...
eqtl = effects %>% dplyr::filter(eqtl_id == "chr2_160468964_A_T.ENSG00000153250")
